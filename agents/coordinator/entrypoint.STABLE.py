...
CRITICAL: Output ONLY the raw JSON object. No ```json``` fences. No text before or after. Just the JSON.
"""
...
analysis_agent = Agent(
    model=BedrockModel(model_id="us.meta.llama4-maverick-17b-instruct-v1:0", region_name=AWS_REGION),
...
def invoke(payload, context):
    ...
    # Build conversation history for context (last 6 turns max to keep tokens manageable)
    history = payload.get("history", [])  # list of {"role": "user"|"assistant", "content": "..."}
    history = history[-6:] if len(history) > 6 else history

    # Build coordinator with or without web_search tool
    tools = [get_available_kpis, data_specialist, analysis]
    ...
    # If analysis tool ran, use its result directly — skip coordinator re-processing (~10-15s saved)
    with _timing_lock:
    ...
    def _unwrap(text: str) -> dict | None:
        """Try to parse text as JSON, unwrapping code fences if needed."""
        t = text.strip()
        # Strip ```json ... ``` fence
    ...
    def _resolve(parsed: dict) -> dict:
        ...
        # Case 1: response is a dict already (coordinator returned object not string)
        ...
        # Case 2: response is a JSON string containing another object
