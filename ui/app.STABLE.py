        ...
        with st.chat_message("assistant"):
            ...
            with st.expander("🔍 Full Timing Breakdown", expanded=False):
                ...
                if coordinator_ms:
                    ...
                    rows.append(f"| 🎯 Coordinator (Haiku 4.5) — model | {coord_model_ms} ms |")
                for e in events:
                    ...
                    if label == 'lambda:get_available_kpis':
                        ...
                        rows.append(f"| {prefix} Nova 2 Lite web search | {ms} ms |")
                    elif label == 'agent:data_specialist':
                        rows.append(f"| 🔬 Data Specialist (Sonnet 4.5) — total | {ms} ms |")
                    elif label == 'agent:analysis':
                        rows.append(f"| 📊 Analysis Agent (Maverick 17B) — total | {ms} ms |")
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ New Conversation", use_container_width=True):
            ...
            st.session_state.stage_latencies = {}
            st.session_state.suggested_questions = []
            logger.info(f"Conversation reset. Old session: {old_session_id}, New session: {st.session_state.session_id}")
            st.rerun()
